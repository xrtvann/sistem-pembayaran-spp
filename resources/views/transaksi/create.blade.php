...
<div class="max-w-xl mx-auto p-6">
    ...
    <form action="{{ route('transaksi.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" value="Rp{{ number_format($pembagian->spp->nominal, 0, ',', '.') }}" disabled class="w-full border-gray-300 rounded mt-1">
        </div>

        <div class="mb-4">
            <label for="total_bayar" class="block text-sm font-medium">Jumlah Bayar</label>
            <input type="number" name="total_bayar" id="total_bayar" class="w-full border-gray-300 rounded mt-1" required>
        </div>

        <div class="mb-4">
            <label for="tanggal_bayar" class="block text-sm font-medium">Tanggal Bayar</label>
            <input type="date" name="tanggal_bayar" id="tanggal_bayar" class="w-full border-gray-300 rounded mt-1" required>
        </div>
